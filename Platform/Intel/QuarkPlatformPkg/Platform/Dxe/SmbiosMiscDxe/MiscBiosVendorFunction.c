...
MISC_SMBIOS_TABLE_FUNCTION(MiscBiosVendor)
{
  ...
  //
  AsciiStrToUnicodeStrS ((CHAR8 *) PcdGetPtr (PcdSMBIOSBiosVendor), Vendor, SMBIOS_STRING_MAX_LENGTH);
  if (StrLen (Vendor) > 0) {
  ...
  AsciiStrToUnicodeStrS ((CHAR8 *) PcdGetPtr (PcdSMBIOSBiosReleaseDate), ReleaseDate, SMBIOS_STRING_MAX_LENGTH);
  ...
  *(UINT64 *)&SmbiosRecord->BiosCharacteristics = PcdGet64 (PcdSMBIOSBiosChar);
  ...
  UnicodeStrToAsciiStrS(VersionPtr, OptionalStrStart + VendorStrLen + 1, VerStrLen + 1);
